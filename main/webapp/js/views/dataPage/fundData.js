...
function operateButton(cellvalue, options, rowObject) {
    ...
    var editBtn = "<button type='button' class='btn btn-primary btn-small' data-toggle='modal' data-target='#myModal' id='editorServer' onclick=\"editFund('"
        + c_id + "','"
        + c_name 
        + "')\">编辑</button>";
    return editBtn + "&ensp;<button type='button' class='btn btn-danger btn-small' data-toggle='modal' data-target='#modalDelete'  onclick='deleteFund("+c_id+")'>移除</button>";
}
//编辑
function editFund(c_id,c_name) {
    type=2;
    $("#name").val(c_name);
    $("#hideId").val(c_id);
