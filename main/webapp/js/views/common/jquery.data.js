// jQuery Plugin for lydata admin template
...
(function($) {
    ...
    // here we go!
    $.lydata = function(element, options) {
        ...
        plugin.toggleSidebar = function() {
            ...
            toggleButton.on("click", function(e){
                ...
                if (sidebar.hasClass('hide-sidebar')) {
                    ...
                    $(".sub li a[class='cli-type active']").parent().parent().prev().trigger("click");
                } else {
                    if (breakpoint == "phone") {
                        ...
                        plugin.toggleLeftSidebar();
                        plugin.collapseSideBarNav(false);
                        $(".cli-type").each(function () {
                            $(this).css("background",'');
                        });
                        $(".sub li a[class='cli-type active']").css("background","#1c202a");
                    }
                }
        ...
        plugin.sideBarNav = function() {
            ...
            //                _this.next('ul').slideDown(plugin.settings.sideNav.subOpenSpeed, plugin.settings.sideNav.animationEasing);
            //                _this.next('ul').addClass('show');
            //                _this.addClass('expand').removeClass('notExpand');//已经展开
            //                navLi.removeClass('highlight-menu');
        ...
        plugin.panels = function () {
            ...
            panels.each(function( index ) {
                ...
                //不知道为什么要动态生成id覆盖原来的id，绑定download函数的时候不好定位，暂时屏蔽掉
                ...
                if(self.hasClass('downLoad')) {
                    panelControlsRight.append('<a href="#"  title="导出excel"><i class="fa fa-download"></i></a>');
                }
            ...
            if (!$('.page-content-inner').hasClass('notSortable')) {
                ...
                if (undefined == $(this).data('resetPage')) {
                    // reset panel position for page
                    $('.reset-layout').click(function(e) {
                        bootbox.confirm({
                            message: "注意！此操作将重置界面布局排版，刷新界面",
                            title: "您是否确认？",
    ...
    $.fn.lydata = function(options) {

        // iterate through the DOM elements we are attaching the plugin to
        return this.each(function() {

            // if plugin has not already been attached to the element
            if (undefined == $(this).data('lydata')) {

                // create a new instance of the plugin
                // pass the DOM element and the user-provided options as arguments
                plugin = new $.lydata(this, options);
                plugin.init();
                // store a reference to the plugin object
                // element.data('lydata').publicMethod(arg1, arg2, ... argn) or
                // element.data('lydata').settings.propertyName
                $(this).data('lydata', plugin);

            }
            //if (undefined == $(this).data('ledo')) {
            //    var plugin = new $.lydata(this, options);
            //    plugin.initInnerPage();
                //$(this).data('ledo', plugin);
            //}
            // if plugin has not already been attached to the element
...
//统一的highchart展示tooltip提示格式
function shwoToolTip() {
    if(isClient=="1"){ //国内环境
        return "<span style=\"color:{series.color}\">\u25CF</span><span>{series.name}</span>: <b></b> {point.y}<br>";
    }
    //海外环境  展示数据保留两位小数
    return "<span style=\"color:{series.color}\">\u25CF</span><span>{series.name}</span>: <b></b> {point.y:.2f}<br>";
}

//数字增长变化效果
function numberCountTo() {
    $('.stats-number').countTo({
        speed: 800,
        refreshInterval: 50
    });
}
function numberCountToType(id) {
    $("#"+id).find(".stats-number").countTo({
        speed: 800,
        refreshInterval: 50,
        //格式化展示数字
...
function playLoading(divId) {
    ...
    $("#"+divId).children(".waitMe").css("z-index",0);
    setTimeout(function(){
        hideLoading(divId) ;
    },5000) ;
}
 ...
 * @param time
 */
function playLoadingWithTime(divId,time) {
    hideLeftRightSide();
    closeNodata(divId);
    $("#"+divId).waitMe({
        effect : "rotateplane",
 ...
 * @param divId
 */
function playLoadingWithoutTime(divId) {
    hideLeftRightSide();
    closeNodata(divId);
    $("#"+divId).waitMe({
        effect : "rotateplane",
...
//调整左右菜单加载隐藏 显示
function hideLeftRightSide() {
    var leftSide = $("#sidebar").attr("class");
    if(leftSide && leftSide.indexOf("collapse-sidebar")!=-1){
        $("#sidebar").attr("class","page-sidebar");
        $('.toggle-sidebar>a').trigger("click");
    }
    var $rightSide = $("#toggle-right-sidebar");
    var hideRside = $rightSide.attr("class");
    if(hideRside && hideRside=="hide-right-sidebar"){
        $rightSide.attr("class","");
        $rightSide.trigger("click");
    }
}