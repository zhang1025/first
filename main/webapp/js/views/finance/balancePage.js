...
function initButtonClick() {
    ...
    $("#balanceDataTables tbody").on("click","tr",function () {
        ...
        var st = $(this).find("td").eq(3).html();//结算单位
    ...
    $("#balance").on("click",function () {
        ...
        //填充弹出框信息，吨数 单价等
        $.post(path+"getBalanceTotal",{ids:id,st:$("#hideStName").val()},
            function (data) {
            ...
            $("#freight").val(data.freight); $("#allMoney").val(data.allMoney);
        });
...
//输入结算金额 算出盈亏
function inputYK() {
    var yingkui = parseFloat($("#sjMoney").val())-parseFloat($("#allMoney").val());
    $("#yingkui").val(yingkui.toFixed(2));
}
//根据id查询调运详细信息
